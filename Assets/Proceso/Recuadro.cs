...
public class Recuadro
{
    ...
    public double anchoSupuesto, altoSupuesto;
    ...
    public Recuadro(int indiceContorno, Point[] contorno, float toleranciaLineaRecta, Point2f escalaContornos = default(Point2f))
    {
    ...
    public void DibujarDebug(Mat imagen, Scalar colEscalar, double escalaPosiciones = 1d, int grosorLado = 2, int radioVertices = 10)
    {
        if (imagen == null) return;
        // var colEscalar = new Scalar(col.b * 255, col.g * 255, col.r * 255, col.a * 255);
        foreach (var vertice in verticesCuadrilatero) Cv2.Circle(imagen, vertice*escalaPosiciones, radioVertices, colEscalar);
        foreach (var lin in ladosCuadrilatero) Cv2.Line(imagen, lin.P1*escalaPosiciones, lin.P2*escalaPosiciones, colEscalar, grosorLado);
    }
    ...
    public Mat Normalizar(Mat origen, double maxTamLado = 0f)
    {
        ...
        if (escalaSalida < 1d) vertsOut = vertsOut.Select(e => e * escalaSalida).ToList();
        if (escalaContornos.X != 1f || escalaContornos.Y != 1f)
        {
            vertsIn = vertsIn.Select(e => new Point2f(e.X / escalaContornos.X, e.Y / escalaContornos.Y)).ToList();
        }
